...
create or replace view data_store.report_approved_awards_edi as 
	select pea.*, award.program_year as date_year, app.ext_id as appl_id, p.id as program_id,  p.name_en as program_name_en, p.name_fr as program_name_fr, a.id as agency_id, a.acronym_en agency_name_en, a.acronym_fr as agency_name_fr, o.id as org_id, o.name_en as org_name_en, o.name_fr as org_name_fr, ds_org.city as org_city, ds_org.postal_zip_code as postal_zip_code, award.amount
		from dataset_application app
		join dataset_award award on app.id=award.dataset_application_id
		join dataset ds on app.dataset_id=ds.id
		join dataset_program ds_program on app.dataset_program_id=ds_program.id
...
create or replace view data_store.orgs_with_external_link_num as 
	select org.*, count(org.id) as link_num 
    from data_store.organization as org
		join data_store.entity_link_organization as link
